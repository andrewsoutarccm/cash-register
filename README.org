...
** How to Build
Make sure you have the submodules, then run ~gradle build~ to compile the application. The build outputs are under the ~build/~ directory. If you do not have gradle, you can use the ~gradle~ or ~gradle.bat~ scripts (depending on your platform) to execute the builtin gradle implementation.
** How to Run
